...
public interface WantedVoMapper {
    ...
    @Select({"SELECT w.id, w.brand, u.nickname, u.position, u.avatarUrl FROM wanted w LEFT JOIN record r ON w.id = r.wantedId LEFT JOIN user u ON r.userId = u.id where w.taked = 0 and w.brand = #{brand} and w.end_time > NOW()"})
